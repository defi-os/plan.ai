...
export const SingleViewItem: React.FC<Props> = ({ view, handleEditView, handleDeleteView }) => {
    ...
    viewsService
      ...
      .catch(() => {
        setToastAlert({
          type: "error",
          title: "Error!",
          message: "Couldn't add the open source to favorites. Please try again.",
        });
      });
  };
    ...
    viewsService
      ...
      .catch(() => {
        setToastAlert({
          type: "error",
          title: "Error!",
          message: "Couldn't remove the open source from favorites. Please try again.",
        });
      });
  };
  ...
  return (
    <div className="first:rounded-t-[10px] last:rounded-b-[10px] hover:bg-custom-background-90">
      <Link href={`/${workspaceSlug}/projects/${projectId}/open-source/${view.id}`}>
        <a>
