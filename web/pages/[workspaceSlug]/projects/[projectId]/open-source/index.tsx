...
import { DeleteViewModal, CreateUpdateViewModal, SingleViewItem } from "components/open-source";
// types
import { IView } from "types";
import type { NextPage } from "next";

const ProjectOpenSource: NextPage = () => {
    ...
    <ProjectAuthorizationWrapper
      breadcrumbs={
        <Breadcrumbs>
          ...
          <BreadcrumbItem title={`${activeProject?.name ?? "Project"} Open Source`} />
        </Breadcrumbs>
      }
      right={
      ...
      {views ? (
        views.length > 0 ? (
          <div className="space-y-5 p-8">
            <h3 className="text-2xl font-semibold text-custom-text-100">Open Source</h3>
          ...
          <EmptyState
            title="Get focused with open sourcce"
            description="Open Source aid in saving your issues by applying various filters and grouping options."
            image={emptyView}
            primaryButton={{
