    ...
    <IssueViewContextProvider>
      <ProjectAuthorizationWrapper
        breadcrumbs={
          <Breadcrumbs>
            <BreadcrumbItem
              title={`${activeProject?.name ?? "Project"} Open Source`}
          ...
          >
            {views?.map((view) => (
              <CustomMenu.MenuItem
                key={view.id}
                renderAs="a"
                href={`/${workspaceSlug}/projects/${projectId}/open-source/${view.id}`}
              >
      ...
      >
        {error ? (
          <EmptyState
            image={emptyView}
            title="Open Source does not exist"
            description="The open source you are looking for does not exist or has been deleted."
            primaryButton={{
              text: "View other open sources",
              onClick: () => router.push(`/${workspaceSlug}/projects/${projectId}/open-source`),
            }}
          />
        ) : (
